...
def render_image(img):
    ...
    cmdqueue += format_message(SetQuality, [0x35])
    ...
    cmdqueue += format_message(DrawingMode, [1])
    ...
    (thresh, img) = cv.threshold(img, 127, 255, cv.THRESH_BINARY | cv.THRESH_OTSU)
    cv.imwrite("test_bw.png", img)
    img_ = 255*(img < 128).astype(np.uint8) 
    coords = cv.findNonZero(img_)
    x, y, w, h = cv.boundingRect(coords)
    cv.imwrite("test_bw_crop.png", img[y:y+h,])
    exit(0)
    img = img[y:y+h,]
    print(f"{img.shape=}")
    if header_lines:
...
if not args.eject:
    ...
    workbook = Workbook()
    workbook.LoadFromFile(args.filename)
    ...
    doc = fitz.open("ToPDF.pdf")
    page = doc[0]
    hits = page.search_for("Evaluation Warning : The document was created with Spire.XLS for Python")
    for rect in hits:
        page.add_redact_annot(rect, "")
    page.apply_redactions(images=fitz.PDF_REDACT_IMAGE_NONE)
    doc.save("ToPDF1.pdf", garbage=3, deflate=True)
    pages = convert_from_path("ToPDF1.pdf", size=(PrinterWidth, None), poppler_path=r"c:\Users\zinan\Documents\poppler-24.08.0\Library\bin")
    for page in pages:
        img = np.array(page)
        print_data = print_data + render_image(img)
